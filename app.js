...
function executeSql(rawSql) {
  ...
  if (insertMatch) {
    ...
    if (columns.length !== values.length) return "Ошибка: число колонок и значений не совпадает.";
  ...
  return "Ошибка: неподдерживаемый SQL-запрос. Используйте формат mini-DBM из подсказки.";
}
...
document.getElementById("run-sql").addEventListener("click", () => {
  const result = executeSql(sqlInputEl.value);
...
document.getElementById("reset-db").addEventListener("click", () => {
  ...
  sqlLogEl.textContent = "OK: демо-БД сброшена к начальному состоянию.";
});
